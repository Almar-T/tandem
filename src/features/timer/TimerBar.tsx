...
export function TimerBar() {
  ...
  if (t.awayNotice) {
    return (
      <div className="fixed bottom-24 left-6 z-40 w-72 animate-fade-up">
        <div className="glass overflow-hidden rounded-2xl border border-hearth-border/40 shadow-lg">
          <div className="flex items-start justify-between px-4 py-3">
            <div>
              <div className="mb-0.5 text-xs font-semibold text-hearth-text/60">Back already?</div>
              <div className="text-[11px] leading-relaxed text-hearth-text/50">
                {t.awayNotice}
              </div>
            </div>
            <button
              onClick={t.dismissAwayNotice}
              className="ml-3 mt-0.5 shrink-0 text-[10px] text-hearth-text/30 transition hover:text-hearth-text/60"
            >
              ✕
            </button>
          </div>
