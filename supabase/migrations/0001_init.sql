...
-- `set search_path = public` + schema-qualified insert are REQUIRED: the auth
-- service runs this trigger with a different search path, so an unqualified
-- `profiles` would fail with "Database error saving new user".
